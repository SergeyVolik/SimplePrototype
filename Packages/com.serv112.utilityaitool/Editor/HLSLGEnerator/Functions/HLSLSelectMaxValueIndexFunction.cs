...
namespace SerV112.UtilityAIEditor
{
    public class HLSLSelectMaxValueIndexFunction : HLSLFunction
    {
        ...
        public HLSLSelectMaxValueIndexFunction(int elements, HLSLMinusInfinity minus_inf, StringBuilder decl) : base(decl)
        {
        ...
        protected override string InternalDeclare()
        {
            ...
            builder.AppendLine($"int SelectMaxValueIndex{m_elements}(float values[{m_elements}])");
            builder.AppendLine("{");
            builder.AppendLine($"    float max = {m_MinusInf.Get()};");
